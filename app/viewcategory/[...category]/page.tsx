...
export default async function Home({ params }: { params: { category: string } }) {
  ...
  const appItems = applist.split("\n").map((app, index) => {
    ...
    if (appCategory === decodeURIComponent(category) || appCategory.split("/")[1]?.includes(decodeURIComponent(category))) {
  ...
  const subCategories = Array.from(
    new Set(
      applist
        .split("\n")
        .map((app) => app.split("|")[1])
        .filter((appCategory) => appCategory?.includes("/") && appCategory?.split("/")[1] !== decodeURIComponent(category) && appCategory?.includes(decodeURIComponent(category)))
    )
